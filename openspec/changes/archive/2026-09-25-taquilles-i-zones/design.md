...
### 2026-09-25. La tarea 5.7 se limita a macOS, como está acordado
La tarea pedía comprobar las pruebas en Windows, Linux y macOS, pero el proyecto decidió (`docs/riesgos.md`, T7 y T10; `docs/flujo-de-trabajo.md`) verificar solo en macOS mientras se desarrolla, dejar Linux descartado por ahora y hacer una única verificación en Windows antes de salir a producción. Se reescribe la tarea con ese alcance para no dejarla abierta ni marcarla con un texto que no se cumple. Las pruebas de persistencia usan rutas con `Path.Combine`, nombres de fichero en minúsculas y ningún bloqueo específico de sistema, y entran en la lista de comprobación de Windows (T10). Decidido por la persona responsable el 2026-09-25.
