...
- Cimientos: capas, base cifrada, migraciones, i18n, resultado estructurado, arranque, registro y CI en tres sistemas.
...
| `ui-shell` | Grupos 1, 2.1 a 2.4 (solo curso y cobros, sin licencia ni asistente), 3 (alumno y taquilla), 5, 6 y 7 | Grupo 4 (identidad y tema), y búsqueda por grupo |
| `pantalles-de-domini` (cambio 15, por redactar tras las maquetas) | Pantallas de curso e importes, zonas, taquillas, alumnos y cargos | Pantallas de llaves, incidencias, informes, cierre, copias y licencia |
| No incluidos | `claus`, `incidencies`, `manteniment`, `cursos-i-historial`, `informes-csv`, `copies-de-seguretat`, `llicencies-client`, `configuracio-inicial` | Todo su contenido |

## Orden de las etapas

```
Etapa 0   Preparación técnica   spike (1) + esqueleto y CI (3)
Etapa 1   Cimientos             arquitectura-base
...
|---------------------------|----------------------|-------------|
| La base de datos se crea automáticamente al arrancar si no existe | Pantalla de primera ejecución (empezar de cero o restaurar) | `configuracio-inicial` |
| Sin licencia: no hay prueba, gracia ni solo lectura | Puerta de licencia en cada caso de uso | `llicencies-client` |
| Los manejadores llevan `[RequiresWriteLicense]` o `[AlwaysAvailable]`, pero nada los aplica | La puerta los aplica | `llicencies-client` |
...
Ninguna bloquea el hito 1. El fichero de secretaría, el RGPD y el servidor de licencias afectan a cambios posteriores. **Sí** hace falta:
...
Propuesta de hito 2 (a confirmar tras la demostración): importación de alumnos y de taquillas (con el fichero de secretaría), llaves, dipòsit completo, cierre de curso e informes. Después, copias de seguridad, licencias, configuración guiada, identidad e instalador.
