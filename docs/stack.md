...
- **Multiplataforma real**: todo paquete debe funcionar en Windows, Linux y macOS (x64 y arm64) y se prueba en la matriz de CI.
...
| GitHub y GitHub Actions | **Gratis con el repositorio público** (actualmente lo es) | Si el repositorio pasara a privado: 2.000 minutos al mes en Linux, con Windows a doble coste y macOS a diez veces | Mantener el repositorio público, o reducir la matriz de CI. No hay datos de alumnos en el repositorio. |
...
| Servidor de licencias | Fuera de este repositorio | Alojamiento y dominio son un coste del otro proyecto | Anotado como dependencia externa; no forma parte del coste de este proyecto. |
...
**Control automático en CI:** un paso que lista las licencias de todos los paquetes NuGet y falla si alguno no está en la lista permitida (MIT, Apache 2.0, BSD, ISC, MS-PL). Así una dependencia nueva o un cambio de licencia en una actualización no entra sin que nos enteremos.
...
| Ficheros de exportación | CSV con la biblioteca **CsvHelper** tras `ICsvReader` | *A confirmar* (licencia y versión) | Si no cumpliera los principios, se sustituye tras el puerto. |
...
| Elemento | Decisión | Versión | Notas |
|----------|----------|---------|-------|
| Firma de claves | **NSec.Cryptography** (Ed25519, basada en libsodium) | 26.4.0 | Licencia MIT. **Ed25519 no está integrado en .NET 10**: la propuesta de la biblioteca estándar sigue abierta. Si NSec diera problemas de binarios nativos, la alternativa de reserva es BouncyCastle (código gestionado). |
...
| Integración continua | **GitHub Actions** con matriz `windows-latest`, `ubuntu-latest` y `macos-latest` | El repositorio está en GitHub. Compila y ejecuta todas las pruebas en los tres sistemas. |
...
1. Parámetros de `SQLite3MC.PCLRaw.bundle` para el formato SQLCipher 4 y la derivación de la clave interna.
