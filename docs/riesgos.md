...
| E3 | **Datos del proyecto de licencias** (endpoint, campos, límite de equipos) | Documento de requisitos del servidor y contrato del cliente (`llicencies-client`, tarea 8.6) | Implementar `llicencies-client` | Persona responsable | Pendiente |
...
| T3 | **NSec** (Ed25519) puede tener problemas de binarios nativos en algún sistema | Baja | Medio | Spike; BouncyCastle como reserva ya elegida | `llicencies-client` |
...
| T7 | Un fallo que **solo ocurre en Windows** (rutas, bloqueos de fichero, cultura) y se descubre tarde | Media | Medio | CI con matriz de tres sistemas; equipo Windows para pruebas puntuales (E9) | Etapa 1 |
| T8 | La **clave interna de cifrado** es la misma en todas las instalaciones: no protege frente a quien examine el programa | Cierta | Medio | Límite ya asumido; documento para la dirección (`arquitectura-base`, 9.5) | Antes de instalar en un centro |
...
| P7 | **Coste cero incumplido** sin darnos cuenta: GitHub pasa a privado, se añade un paquete de pago, firma de código | Baja | Alto | Regla en `AGENTS.md`, control en CI, repositorio público, sin firma de código | Continuo |
...
| P10 | **El servidor de licencias** (otro proyecto) se retrasa y bloquea la comercialización | Media | Medio | El cliente funciona sin servidor; contrato mínimo definido | Fuera de este repositorio |
